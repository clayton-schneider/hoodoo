...
const ContactForm = ({
  ...
  return (
    <form
      ...
      className="grid w-full grow grid-cols-1 gap-4 lg:grid-cols-2"
    >
        ...
        >
          ...
          {field.fieldType === "textarea" ? (
            <textarea
              ...
              name={field.name}
              id={field.id}
              className="h-[150px] w-full resize-none border bg-white px-4 pt-2 placeholder:text-black focus:outline-none"
      ...
      {(formState === "ready" || formState === "empty") && (
        <button className="col-span-full border-2 border-transparent bg-cta py-6 px-7 text-center leading-none text-white transition-all duration-300 hover:border-cta hover:bg-transparent hover:text-cta">
          {button}
        </button>
      )}
      {formState === "empty" && (
